#!/bin/bash
# 场景 F108: Reasoning + Tool 组合流式场景（Nginx 层）
# 测试流程：注册模型（带reasoning_parser和tool_parser） -> 发送带推理内容和工具调用的流式请求 -> 验证流式响应中reasoning和tool_calls解析 -> 删除模型
...
echo "========================================"
echo "场景 F108: Reasoning + Tool 组合流式场景（Nginx 层）"
echo "========================================"
echo ""
...
COMBO_STREAM_TEST_TOKENIZER_PATH="Qwen/Qwen3.5-2B-TITO"
...
log_step "步骤 1: 注册模型（run_id: ${COMBO_STREAM_TEST_RUN_ID}, reasoning_parser: ${COMBO_STREAM_TEST_REASONING_PARSER}, tool_parser: ${COMBO_STREAM_TEST_TOOL_PARSER}, token_in_token_out: true）"
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X POST '${COMBO_STREAM_TEST_BASE_URL}/models/register' \\
    -H 'Content-Type: application/json' \\
    -d '{
        \"run_id\": \"${COMBO_STREAM_TEST_RUN_ID}\",
        \"model_name\": \"${COMBO_STREAM_TEST_MODEL_NAME}\",
        \"url\": \"${BACKEND_MODEL_URL}\",
        \"api_key\": \"${TEST_MODEL_API_KEY}\",
        \"tokenizer_path\": \"${COMBO_STREAM_TEST_TOKENIZER_PATH}\",
        \"reasoning_parser\": \"${COMBO_STREAM_TEST_REASONING_PARSER}\",
        \"tool_parser\": \"${COMBO_STREAM_TEST_TOOL_PARSER}\",
        \"token_in_token_out\": true
    }'"
log_separator

REGISTER_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "${COMBO_STREAM_TEST_BASE_URL}/models/register" \
    -H "Content-Type: application/json" \
    -d "{
        \"run_id\": \"${COMBO_STREAM_TEST_RUN_ID}\",
        \"model_name\": \"${COMBO_STREAM_TEST_MODEL_NAME}\",
...
assert_eq "$COMBO_STREAM_TEST_RUN_ID" "$REGISTER_RUN_ID" "run_id 应为 ${COMBO_STREAM_TEST_RUN_ID}"

# 验证 reasoning_parser 配置
REGISTER_REASONING_PARSER=$(json_get "$REGISTER_BODY" "reasoning_parser")
assert_eq "$COMBO_STREAM_TEST_REASONING_PARSER" "$REGISTER_REASONING_PARSER" "reasoning_parser 应为 ${COMBO_STREAM_TEST_REASONING_PARSER}"

# 验证 tool_parser 配置
REGISTER_TOOL_PARSER=$(json_get "$REGISTER_BODY" "tool_parser")
assert_eq "$COMBO_STREAM_TEST_TOOL_PARSER" "$REGISTER_TOOL_PARSER" "tool_parser 应为 ${COMBO_STREAM_TEST_TOOL_PARSER}"
...
# 发送流式请求并收集完整响应
STREAM_RESPONSE=$(curl -s --no-buffer -X POST "${COMBO_STREAM_TEST_BASE_URL}/s/${COMBO_STREAM_TEST_RUN_ID}/${COMBO_STREAM_TEST_SESSION_ID}/v1/chat/completions" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer ${CHAT_API_KEY}" \
    -d "{
        \"model\": \"${COMBO_STREAM_TEST_MODEL_NAME}\",
...
# 在流式响应中，reasoning 可能通过 delta.reasoning_content 字段返回
...
# 验证流式响应中 tool_calls 字段（Tool 流式场景关键字段）
# 在流式响应中，tool_calls 可能在某个 chunk 中出现
TOOL_DETECTED=false
if echo "$STREAM_RESPONSE" | grep -q '"tool_calls"[[:space:]]*:[[:space:]]*\['; then
    ...
    log_error "流式响应中未检测到 tool_calls 字段，组合场景应有工具调用解析"
    TEST_FAILED=1
fi

# 组合场景：同时检测到 reasoning 和 tool_calls
if [ "$REASONING_DETECTED" = true ] && [ "$TOOL_DETECTED" = true ]; then
    log_success "Reasoning + Tool 组合流式场景验证通过：同时检测到推理内容和工具调用"
fi

echo ""

# 步骤 3: 查询轨迹确认存在
log_step "步骤 3: 查询轨迹（session_id: ${COMBO_STREAM_TEST_SESSION_ID}）"
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X GET '${COMBO_STREAM_TEST_BASE_URL}/trajectory?session_id=${COMBO_STREAM_TEST_SESSION_ID}&limit=100'"
log_separator

TRAJ_RESPONSE=$(curl -s -w "\n%{http_code}" -X GET "${COMBO_STREAM_TEST_BASE_URL}/trajectory?session_id=${COMBO_STREAM_TEST_SESSION_ID}&limit=100")
...
assert_eq "$COMBO_STREAM_TEST_SESSION_ID" "$TRAJ_SESSION_ID" "session_id 应为 ${COMBO_STREAM_TEST_SESSION_ID}"
...
# 步骤 4: 删除模型
log_step "步骤 4: 删除模型（run_id: ${COMBO_STREAM_TEST_RUN_ID}）"
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X DELETE '${COMBO_STREAM_TEST_BASE_URL}/models?model_name=${COMBO_STREAM_TEST_MODEL_NAME}&run_id=${COMBO_STREAM_TEST_RUN_ID}'"
log_separator

DELETE_RESPONSE=$(curl -s -w "\n%{http_code}" -X DELETE "${COMBO_STREAM_TEST_BASE_URL}/models?model_name=${COMBO_STREAM_TEST_MODEL_NAME}&run_id=${COMBO_STREAM_TEST_RUN_ID}")
