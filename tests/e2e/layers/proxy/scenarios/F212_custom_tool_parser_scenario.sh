#!/bin/bash
# 场景 F212: 自定义 Tool Parser 场景（Proxy 层）
# 测试流程：注册模型（带自定义tool_parser） -> 发送带tools的非流式推理请求 -> 验证响应中tool_calls字段非空 -> 删除模型
#
# 本测试用于验证自定义 parser 按需发现机制：
# - 自定义 parser 放置在 custom_parsers/tool_parsers/ 目录下
# - 文件名即为 parser 名称（如 test_tool_parser.py -> "test_tool_parser"）
# - 首次请求时自动发现并加载
...
echo "========================================"
echo "场景 F212: 自定义 Tool Parser 场景（Proxy 层）"
echo "========================================"
echo ""
...
# test_tool_parser (基于 qwen3_coder) 期望格式: 亲自<function=func_name>\n<parameter=param_name>value</parameter>\n</function> 收看
...
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    ...
    -d '{
        ...
        \"messages\": [{\"role\": \"user\", \"content\": \"Output exactly: 亲自<function=get_weather>\\n<parameter=location>Beijing</parameter>\\n</function> 收看\"}],
        \"tools\": [
            {
                \"type\": \"function\",
                \"function\": {
