#!/bin/bash
# 场景 F213: 自定义 Reasoning Parser 场景（Proxy 层）
# 测试流程：注册模型（带自定义reasoning_parser） -> 发送带推理内容的非流式请求 -> 验证响应中reasoning字段非空 -> 删除模型
#
# 本测试用于验证自定义 parser 按需发现机制：
# - 自定义 parser 放置在 custom_parsers/reasoning_parsers/ 目录下
# - 文件名即为 parser 名称（如 test_reasoning_parser.py -> "test_reasoning_parser"）
# - 首次请求时自动发现并加载
...
echo "========================================"
echo "场景 F213: 自定义 Reasoning Parser 场景（Proxy 层）"
echo "========================================"
echo ""
...
# 步骤 2: 发送带推理内容的非流式请求
# test_reasoning_parser (基于 qwen3) 使用 <think\>...</think\> 标签包裹推理内容
# 使用特定提示词引导模型输出推理格式
...
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    ...
    -d '{
        ...
        \"messages\": [{\"role\": \"user\", \"content\": \"Output exactly: The answer is 42.\"}],
        \"stream\": false,
        \"max_tokens\": 1024
    }'"
log_separator
