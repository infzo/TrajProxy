...
graph TB
    ...
    subgraph "Docker容器"
        ...
        subgraph "traj_proxy容器"
            ...
            subgraph "Ray Workers"
                PCW0[ProxyCoreWorker-0<br/>8000]
                PCW1[ProxyCoreWorker-1<br/>8001]
                TPW0[TranscriptProvider<br/>9000]
            end
        end
    end
    ...
    style PCW0 fill:#ffcc80
    style PCW1 fill:#ffcc80
    style TPW0 fill:#ffe0b2
...
| ProxyCore | 8000-8010 | 核心代理服务，支持多 Worker 并发 |
| TranscriptProvider | 9000-9005 | 转录服务提供者 |
...
编辑 `config.yaml`，配置 LLM 推理服务地址和数据库连接：
...
ray:
  num_cpus: 4
  working_dir: "/app/traj_proxy"  # 容器内路径
  pythonpath: "/app/traj_proxy"    # 容器内 PYTHONPATH

# 数据库配置
database:
  url: "postgresql://llmproxy:dbpassword9090@localhost:5432/litellm"
```
...
# 检查可用模型
curl http://localhost:8000/proxy/v1/models
```
...
| traj_proxy | 8000-8010, 9000-9005 | TrajProxy Workers |
...
Worker 配置文件位于 `traj_proxy/config.yaml`，主要配置项：
...
model_list:
  ...
  - model_name: "*"
    litellm_params:
      model: "openai/*"
      api_base: http://traj_proxy:8000/proxy/v1  # 内部 ProxyCore 服务
      api_key: "sk-1234"
  - model_name: "*"
    litellm_params:
      model: "openai/*"
      api_base: http://traj_proxy:8001/proxy/v1  # 第二个 ProxyCore 实例
      api_key: "sk-1234"
...
```bash
curl --location 'http://localhost:8000/proxy/v1/chat/completions' \
  --header 'Content-Type: application/json' \
  -H "x-session-id: app_001#sample_001#task_001" \
  --data '{
    "model": "qwen3.5-2b",
...
```bash
curl "http://localhost:8000/transcript/trajectory?session_id=app_001#sample_001#task_001"
```
...
### TranscriptProvider（端口 9000-9005）
...
│   │   ├── worker.py              # TranscriptProviderWorker
│   │   ├── provider.py            # 对话历史提供者
│   │   └── routes.py              # API路由
│   ├── store/                      # 存储模块
│   │   └── database.py            # 数据库管理器
...
| 8000-8010 | 8000-8010 | TrajProxy Workers | ProxyCore Worker（多实例） |
| 9000-9005 | 9000-9005 | TranscriptProvider Workers | 对话历史服务 |
| 9090 | 9090 | Prometheus | 监控面板 |

**使用建议**：
- 客户端请求统一通过 `4000` 端口（LiteLLM）访问
- 内部服务端口（8000+、9000+）用于直接调用和调试
