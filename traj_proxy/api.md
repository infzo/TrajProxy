...
- **端口**:
  - ProxyCoreWorker: `4000` (聊天和模型管理)
  - TranscriptProviderWorker: `8000` (轨迹记录查询)
...
### TranscriptProvider 健康检查

检查转录服务的运行状态。

**接口地址**: `GET /transcript/health`
...
| x-session-id | string | 否 | 会话 ID，用于请求追踪，格式: `app_id#sample_id#task_id` |
...
**cURL 示例**:
```bash
curl -X POST http://localhost:4000/proxy/v1/chat/completions \
...
**cURL 示例**:
```bash
curl "http://localhost:8000/transcript/trajectory?session_id=app_001#sample_001#task_001&limit=100"
```
...
TRANSCRIPT_BASE_URL = "http://localhost:8000"

# 1. 聊天补全
response = requests.post(
    f"{PROXY_BASE_URL}/proxy/v1/chat/completions",
    headers={
...
# 2. 注册模型
response = requests.post(
    f"{PROXY_BASE_URL}/proxy/models/register",
    json={
        "model_name": "gpt-4",
...
response = requests.get(
    f"{TRANSCRIPT_BASE_URL}/transcript/trajectory",
    params={
        "session_id": "app_001#sample_001#task_001",
        "limit": 100
    }
...
本系统的 `/proxy/v1/chat/completions` 接口与 OpenAI API 完全兼容，可以直接使用 OpenAI 官方 SDK 进行调用：

```python
from openai import OpenAI

client = OpenAI(
    base_url="http://localhost:4000/proxy/v1",
    api_key="any-key"  # TrajProxy 可选使用该字段进行认证
)
