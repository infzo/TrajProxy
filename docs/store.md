...
├── request_repository.py       # 请求轨迹 CRUD
...
ProxyWorker
  ...
  ├── DatabaseManager (连接池)
  │       │
  │       ├── RequestRepository ─────► request_records 表
  │       │       ▲
  │       │       │ insert() / get_by_session()
  │       │       │
  │       │  TranscriptProvider (薄封装)
  │       │       ▲
  │       │       │ get_trajectory()
  │       │       │
...
| `RequestRepository` | 请求轨迹记录的插入和查询 |
...
class RequestRecord:
    ...
    prompt_text: Optional[str]         # 文本 prompt
    token_ids: Optional[List[int]]     # Token ID 列表
    # ... 完整字段见 database.md
```
...
| 方法 | 签名 | 说明 |
|------|------|------|
| `insert` | `async (context: ProcessContext, tokenizer_path: Optional[str]) -> None` | 插入轨迹记录 |
| `get_by_session` | `async (session_id: str, limit: int = 100) -> List[Dict]` | 按 session_id 查询，按 start_time 倒序 |
...
transcript_provider = TranscriptProvider(repo)    # 4. 创建 TranscriptProvider

await processor_manager.start_sync()             # 5. 启动 LISTEN + 兜底同步
# 6. 注册预置模型（内存，不持久化）
```
...
| 调用方 | 使用的组件 | 方法 |
|--------|-----------|------|
| `Processor` | `RequestRepository` | `insert()` |
| `StreamingProcessor` | `RequestRepository` | `insert()` |
| `ProcessorManager` | `ModelRepository` | `register()`, `unregister()`, `get_all()`, `get_by_key()` |
| `ProcessorManager` | `NotificationListener` | `start()`, `stop()` |
| `TranscriptProvider` | `RequestRepository` | `get_by_session()` |
| `routes.py` | `TranscriptProvider` | `get_trajectory()` |
