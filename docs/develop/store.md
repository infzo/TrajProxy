...
ProxyWorker
  ...
  │       │       ▲
  │       │       │ insert() / get_by_session() / get_metadata_by_session() / get_statistics()
  │       │       │
...
| 方法 | 签名 | 说明 |
|------|------|------|
| `insert` | `async (context: ProcessContext, tokenizer_path: Optional[str]) -> None` | 事务双写 metadata + details 表 |
...
| `TrajectoryProvider` | `RequestRepository` | `get_by_session()` |
| `routes.py` | `TrajectoryProvider` | `get_trajectory()` |
